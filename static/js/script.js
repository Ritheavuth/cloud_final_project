...
// Atler User of New Room Created
const newRoomSocket = new WebSocket(
  "ws://" + window.location.host + "/ws/new_room/"
);

newRoomSocket.onopen = function (event) {
  console.log("WebSocket connection established.");
};

newRoomSocket.onmessage = function (event) {
  const data = JSON.parse(event.data);
  if (data.type === "room_created") {
    let room = document.getElementById("room-list");

    room.insertAdjacentHTML(
      "afterbegin",
      `
      <div id="roomListRoom" class="roomListRoom">
        <div class="roomListRoom__header">
          ...
          >
            ...
            <span>Host @${data.user.username}</span>
          </a>
        ...
        <div class="roomListRoom__content">
          <a href="/room/${data.room_id}">${data.name}</a>
        </div>
        <div class="roomListRoom__meta">
          <a href="/room/${data.room_id}" class="roomListRoom__joined">
            <svg
              version="1.1"
            ...
            ${data.room_participants_count} Joined
          </a>
...
newRoomSocket.onclose = function (event) {
